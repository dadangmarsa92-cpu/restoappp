...
export default function AdminTables() {
  ...
  const fetchTables = async () => {
    ...
    // Sort by name if alphabetical/numeric
  ...
  return (
    <div className="pb-32">
      <div className="flex justify-between items-end mb-10">
        <div>
          <h2 className="text-3xl font-extrabold tracking-tight mb-2">Manajemen Meja</h2>
          <p className="text-secondary font-medium">Atur status dan daftar meja di restoran Anda.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 gap-4">
        {tables.map((table) => (
          <div key={table.id} className="bg-surface-container-lowest p-6 rounded-3xl flex justify-between items-center shadow-sm border border-outline-variant/10">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="w-10 h-10 bg-surface-container-high hover:bg-surface-container-highest rounded-full flex items-center justify-center transition-colors"
                title="Ganti Status"
              >
      ...
      <AnimatePresence>
        {showAdd && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-6">
            ...
            >
              <h3 className="text-2xl font-black font-headline mb-6">Tambah Meja</h3>
              <form onSubmit={handleAddTable} className="space-y-6">
                <div>
                  <label className="block text-xs font-bold text-secondary uppercase tracking-widest mb-2">Nama/Nomor Meja</label>
                  <input
                    required
                    type="text"
                    value={newTableName}
                    onChange={(e) => setNewTableName(e.target.value)}
                    placeholder="Contoh: 01 atau VIP-1"
